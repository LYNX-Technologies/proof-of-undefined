...
contract BiometricToken is ERC721, ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    mapping(uint256 => bool) private existingURIs;
    mapping(uint256 => string) private biometricsStorage;

    constructor() ERC721("BiometricData", "CE") {
    }
    ...
    function safeMint(address to, string memory uri, string memory encryptionKey) public onlyOwner returns (uint256) {
        ...
        biometricsStorage[newItemId] = encryptionKey;
